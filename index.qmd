...
# Scatter points
plt.scatter(dreamy['patient'], dreamy['post_surgical_score'],
            color='#4E79A7', marker='o', label='Doc Dreamy (O)', alpha=0.7)
plt.scatter(duck['patient'], duck['post_surgical_score'],
            color='#E15759', marker='^', label='Doc Duck (△)', alpha=0.7)

# Add mean lines
plt.axhline(mean_dreamy, color='blue', linestyle='--', linewidth=1)
plt.axhline(mean_duck, color='coral', linestyle='--', linewidth=1)

# Annotate mean values
plt.text(patients_df['patient'].max() + 1, mean_dreamy,
         f"Mean: {mean_dreamy:.2f} ○",
         color='blue', fontsize=10, va='center')
plt.text(patients_df['patient'].max() + 1, mean_duck,
         f"Mean: {mean_duck:.2f} △",
         color='coral', fontsize=10, va='center')

# Labels and title
plt.xlabel("Patient Number", fontsize=12)
plt.ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=12)
plt.title("Post-Surgical Symptom Score by Patient and Doctor",
...
# Set font to DejaVu Sans for proper glyph rendering
plt.rcParams['font.family'] = 'DejaVu Sans'
...
# Scatter points
plt.scatter(dreamy_rand['patient'], dreamy_rand['post_surgical_score'],
            color='#4E79A7', marker='o', label='Doc Dreamy (○)', alpha=0.7)
plt.scatter(duck_rand['patient'], duck_rand['post_surgical_score'],
            color='#E15759', marker='^', label='Doc Duck (△)', alpha=0.7)

# Add mean lines
plt.axhline(mean_dreamy_rand, color='blue', linestyle='--', linewidth=1)
plt.axhline(mean_duck_rand, color='coral', linestyle='--', linewidth=1)

# Annotate mean values
plt.text(patients_randomized_df['patient'].max() + 1, mean_dreamy_rand,
         f"Mean: {mean_dreamy_rand:.2f} ○",
         color='#4E79A7', fontsize=10, va='center')
plt.text(patients_randomized_df['patient'].max() + 1, mean_duck_rand,
         f"Mean: {mean_duck_rand:.2f} △",
         color='#E15759', fontsize=10, va='center')
...
# Load the original dataset & filter for severity betweem -1 and 1
filtered_df = patients_df[(patients_df['severity'] >= -1) & (patients_df['severity'] <= 1)]
...
# Calculate means
mean_dreamy = filtered_df[filtered_df['doctor_name'] == 'Doc Dreamy']['post_surgical_score'].mean()
mean_duck = filtered_df[filtered_df['doctor_name'] == 'Doc Duck']['post_surgical_score'].mean()

# Set font for glyph support
plt.rcParams['font.family'] = 'DejaVu Sans'
...
# Scatter points
plt.scatter(dreamy['severity'], dreamy['post_surgical_score'],
            color='#4E79A7', marker='o', label='Doc Dreamy (○)', alpha=0.7)
plt.scatter(duck['severity'], duck['post_surgical_score'],
            color='#E15759', marker='^', label='Doc Duck (△)', alpha=0.7)

# Add mean horizontal lines
plt.axhline(mean_dreamy, color='#4E79A7', linestyle='--', linewidth=1)
plt.axhline(mean_duck, color='#E15759', linestyle='--', linewidth=1)

# Annotate mean values
plt.text(patients_randomized_df['severity'].max() + 0.2, mean_dreamy,
         f"Mean: {mean_dreamy:.2f} ○",
         color='#4E79A7', fontsize=10, va='center')
plt.text(patients_randomized_df['severity'].max() + 0.2, mean_duck,
         f"Mean: {mean_duck:.2f} △",
         color='#E15759', fontsize=10, va='center')

# Add shaded region for severity between -1 and 1
plt.axvspan(-1, 1, color='gray', alpha=0.15, label='Severity [-1, 1]')

# Labels and title
plt.xlabel("Initial Patient Severity", fontsize=12)
plt.ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=12)
plt.title("Post-Surgical Symptom Score by Patient Severity",
          fontsize=14, weight='bold')
